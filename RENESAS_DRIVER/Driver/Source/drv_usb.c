...
{
    uint16_t chunk;
    uint8_t  pipe = USB_PIPE_CDC_DEV_BULK_IN;
    ...
    /* Fire-and-forget logger: proactively recover endpoint state so a prior
     * transient NRDY/timeout does not wedge subsequent log writes. */
    usb_pipe_set_pid(pipe, USB_CTR_PID_NAK);
    usb_pipe_clear_buffer(pipe);
    usb_clear_bemp(pipe);
    usb_clear_nrdy(pipe);
    usb_fifo_select_pipe(pipe, USB_DIR_IN);

    /* Send up to one max-packet-size chunk (64 bytes). Longer messages are
     * truncated; acceptable for debug/diagnostic logging use cases. */
    chunk = (uint16_t)(len > 64U ? 64U : len);

    if (usb_fifo_write(data, chunk) != DRV_OK)
    {
        usb_device_abort_bulk_in(pipe);
        return DRV_TIMEOUT;
    }

    /* BVAL signals end-of-transfer; required for short packets and safe
     * for full 64-byte packets when only one packet is sent. */
    USB_CFIFOCTR |= USB_FIFOCTR_BVAL;
    usb_clear_bemp(pipe);
    usb_clear_nrdy(pipe);
    usb_pipe_set_pid(pipe, USB_CTR_PID_BUF);
...
drv_status_t USB_Dev_Printf_Log(const char *fmt, ...)
{
    ...
    return USB_Dev_Write_Log((const uint8_t *)buf, (uint32_t)n);
}
